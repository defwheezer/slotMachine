...
def draw_player_data(bank):
   textSurf = a_sys_font.render("Bank: "+("{:.2f}".format(bank)), 1, fg)
   textRect = textSurf.get_rect()
   textRect.center = (WIDTH/6, HEIGHT-40)
...
def main():
   ...
   bank = 0 #total bank, start with 0
      ...
      #check IO pin for button press
       ...
       if input_state == False:
          ...
          if bank > 1:
             if bribe > 2:
                adjBribe = 2
                textSurf = d_sys_font.render("* Up to 2 credit Maximum", 1, fg, bg)
                textRect = textSurf.get_rect()
       ...
       textSurf = b_sys_font.render("Soul Cost: 1 Quarter", 1, fg)
       textRect = textSurf.get_rect()
       textRect.center = ((WIDTH/2),(HEIGHT/6))
       screen.blit(textSurf, textRect)
       
       textSurf = b_sys_font.render("Press 'Green Button' To Sell Soul", 1, fg)
       textRect = textSurf.get_rect()
       textRect.center = ((WIDTH/2),(HEIGHT/2.5))
       screen.blit(textSurf, textRect)

       textSurf = b_sys_font.render("Press 'Red Button' To Cash Out", 1, fg)
       textRect = textSurf.get_rect()
